...
async function getLastYoutubeCode(){
    return await fetch(`https://corsproxy.io/?url=${encodeURIComponent("https://youtube.com/results")}`).then(async r => await r.text().then(body =>
    {
        var b = document.createElement("html");
        b.innerHTML = body;
        return b.children[0].getElementsByTagName("link")[0].href.replace("https://www.youtube.com/s/desktop/","").replace("/img/logos/favicon.ico","");
    }));
}
